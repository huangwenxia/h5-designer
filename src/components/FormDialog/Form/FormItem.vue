<template>
    <div>
        ...
        <el-input v-model="form[item.prop]" :readonly='item.readonly' v-only-number="item.onlyNumber"  :style="item.style" :type="item.proptype || 'text'" :label="item.label" v-if="typeof(item.formtype)=='undefined' || item.formtype=='input'" clearable></el-input>
        ...
        <template v-if="item.formtype=='textarea'">
            <el-input type="textarea" :label="item.label" v-model="form[item.prop]"  :style="item.style" :autosize="{ minRows: item.minRows || 4}"></el-input>
        </template>
        ...
        <!-- formtype 为 autocomplete-->
        <template v-if="item.formtype=='autocomplete'">
            <div class="my-autocomplete">
                <div class="date-label is-active">{{ item.label }}</div>
                <el-autocomplete v-model="form[item.prop]" :fetch-suggestions="autoSearch"  :placeholder="$t('请输入或者选择内容')"  :value-key="item['valueKey'] || 'value'" style="width: 100%;" clearable>
                    <template slot-scope="scope">
                        <div style="line-height: 1.4;padding-top:5px;">{{scope.item[item['labelKey'] || 'name']}}</div>
                        <div style="line-height: 1.4;padding-bottom:5px;color:#999;">{{scope.item[item['valueKey'] || 'value']}}</div>
                    </template>
                </el-autocomplete>
            </div>
        </template>
        ...
        <template v-if="item.formtype=='radio'">
            <div class="date-label is-active">{{ item.label }}</div>
             <el-radio-group v-model="form[item.prop]">
                ...
                <el-radio v-for='radio in item.radioList' :key='radio.value' :label="radio.value">{{radio.name||radio.label || radio[item.labelKey]}}</el-radio>
              </el-radio-group>
        </template>
        
        <template v-if="item.formtype=='file'">
            <div v-if="item.label" style="top: 13px;" slot="prefix" class="effect-input__label" :class="{'is-active':true}">{{ item.label }}</div>
            <FileUploader style="position: relative;top:25px;" :files.sync="form[item.prop]" :single="true" :autoUpload="item.hasOwnProperty('autoUpload')?item.autoUpload:true"></FileUploader>
        </template>
...
export default{
    ...
    created(){
        ...
        if(this.item.defaultValue){
            this.form[this.item.prop] = this.item.defaultValue;
        }
    },
    methods:{
        autoSearch(query, cb){//纯前端搜索
            let currSelectList = deepClone(this.item.options);
            let result = currSelectList.filter(item=>{
                let qryString = item[this.item['searchKey'] || 'name'];
                if(qryString && query && (qryString.toLowerCase().indexOf(query.toLowerCase())>-1 || qryString.toLowerCase().indexOf(query.toLowerCase())>-1)){
                    return  item;
                }
            });
            if(result.length){
