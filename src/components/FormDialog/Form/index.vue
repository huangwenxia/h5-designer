    ...
    <el-form class="form" ref="form" :model="form" :rules="rules" label-width="160px" @submit.native.prevent v-loading="loading">
        <template v-for="(item,index) in schema">
            <el-form-item :label="item.label" :key="index" :model="form" :prop="item.prop" :schema='schema' v-if="!isHidden(item)">
                <div class="date-label is-active" v-if="item.value">{{ item.label }}</div>
                <span v-if="item.value" v-html="item.value"></span>
                <FormItem :form="form" :item="item" @change="onChange" v-else></FormItem>
            </el-form-item>
        </template>
        
        <el-form-item class="btn-area">
                <el-button @click="cancel">{{$t('actions.cancel')}}</el-button>
                <el-button class="btn-common" type="primary" @click="submit" native-type="submit">{{$t('actions.confirm')}}</el-button>
            </el-form-item>
    </el-form>
</template>
    ...
    methods:{
        ...
        submit(){
            ...
            if(!this.api){
                ...
                return this.$emit('submit',postData,null,next)
            }
            this.loading=true;
            ...
            if(postData.id){
                this.api.update(postData).then(res=>{
                    this.loading=false;
                    this.$message({type: 'success',message: this.$t('操作成功'),duration:1500,showClose: true,});
                    this.$emit('submit',this.form,res)
                }).finally(()=>{
                    this.loading=false;
                })
            }else{
